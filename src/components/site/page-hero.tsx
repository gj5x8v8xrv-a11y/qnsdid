...
export function PageHero({
  ...
  return (
    <section className="page-shell pb-5 pt-3 sm:pb-14 sm:pt-10">
      <div className="overflow-hidden rounded-[2rem] border border-black/8 bg-white px-4 py-4 text-black shadow-[0_24px_70px_rgba(15,23,42,0.06)] sm:rounded-[2.5rem] sm:px-10 sm:py-12 lg:px-12 lg:py-14">
        <div className="grid gap-5 sm:gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)] lg:items-start">
          <div>
            ...
            <h1 className="mt-3 text-[length:var(--home-hero-title-size,1.42rem)] leading-[1.18] tracking-[-0.03em] text-black sm:mt-6 sm:text-[2.8rem] lg:text-[3.55rem]">
              {title}
            </h1>
            <div
              className={`mt-3 max-w-3xl text-[length:var(--home-body-text-size,13px)] leading-6 text-muted sm:mt-5 sm:text-[17px] sm:leading-8 ${descriptionClassName || ""}`}
            >
              {description}
            </div>

            {actions ? (
              <div className={`mt-5 flex flex-wrap gap-2.5 sm:mt-7 sm:gap-3 ${actionsClassName || ""}`}>
                {actions}
              </div>
            ) : null}
